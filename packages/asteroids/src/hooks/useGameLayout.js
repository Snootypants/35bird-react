...
export const useGameLayout = (
  ...
  useEffect(() => {
    ...
    const updateGameLayout = () => {
      ...
      const containerHeight = host.clientHeight || window.innerHeight;
      ...
      if (playHeight > containerHeight) {
        playHeight = containerHeight;
        playWidth = Math.round(playHeight * ASPECT_RATIO);
      }

      const playX = Math.round((containerWidth - playWidth) / 2);
      const playY = Math.round((containerHeight - playHeight) / 2);
    ...
    const observer =
      typeof ResizeObserver !== 'undefined'
        ? new ResizeObserver(() => updateGameLayout())
        : null;
