...
import { WORLD_WIDTH, WORLD_HEIGHT, STAR_COUNT, STAR_MIN_BRIGHTNESS, STAR_MAX_BRIGHTNESS,
         ...
         WAVE_SPEED_CURVE_TAU,
         WAVE_SPEED_MIN_ADD,
         WAVE_SPEED_MAX_ADD } from '../utils/constants.js';
...
export function useGameWorld({
  ...
  const computeWaveSpeedRange = useCallback((stageNumber) => {
    ...
    const minSpeed = ASTEROID_SPEED + progress * WAVE_SPEED_MIN_ADD;
    const maxSpeed = ASTEROID_SPEED + progress * WAVE_SPEED_MAX_ADD;
    return {
      minSpeed,
  ...
  const sampleSpeed = useCallback((minSpeed, maxSpeed) => {
    if (maxSpeed <= minSpeed) {
      return minSpeed;
    }

    const mean = (minSpeed + maxSpeed) / 2;
    const stdDev = (maxSpeed - minSpeed) / 6; // ~99% of samples within range
    for (let i = 0; i < 6; i += 1) {
      const u1 = Math.random();
      const u2 = Math.random();
      const z0 = Math.sqrt(-2 * Math.log(u1)) * Math.cos(2 * Math.PI * u2);
      const candidate = mean + stdDev * z0;
      if (candidate >= minSpeed && candidate <= maxSpeed) {
        return candidate;
      }
    }

    return Math.min(maxSpeed, Math.max(minSpeed, mean));
  }, []);
  ...
  const initializeAsteroids = useCallback((count = INITIAL_ASTEROID_COUNT, explicitRange) => {
    const initialAsteroids = [];
    let speedRange = explicitRange;
    if (modeRef?.current === 'waves') {
      const stageNumber = stageRef.current ?? 1;
      if (!speedRange) {
        speedRange = computeWaveSpeedRange(stageNumber);
      }
      waveSpeedRangeRef.current = speedRange;
    }
    for (let i = 0; i < count; i++) {
      const x = Math.random() * WORLD_WIDTH;
      const y = Math.random() * WORLD_HEIGHT;
      const asteroid = new Asteroid(x, y);
      if (modeRef?.current === 'waves' && waveSpeedRangeRef.current) {
        const { minSpeed, maxSpeed } = waveSpeedRangeRef.current;
        const speed = sampleSpeed(minSpeed, maxSpeed);
        const heading = Math.random() * Math.PI * 2;
        asteroid.vx = Math.cos(heading) * speed;
        asteroid.vy = Math.sin(heading) * speed;
      }
      initialAsteroids.push(asteroid);
    }
    asteroidsRef.current = initialAsteroids;
  }, [modeRef, stageRef, computeWaveSpeedRange, sampleSpeed]);
  ...
  const clearAsteroids = useCallback(() => {
    asteroidsRef.current = [];
    asteroidCountsRef.current = { large: 0, medium: 0, small: 0 };
  }, []);
  ...
  const clearHyperCountdown = useCallback(() => {
    ...
    if (hyperCountdownStageTimeoutRef.current) {
      clearTimeout(hyperCountdownStageTimeoutRef.current);
      hyperCountdownStageTimeoutRef.current = null;
    }
  ...
  const startNewStage = useCallback((stageNumber, asteroidCount) => {
    ...
    initializeAsteroids(asteroidCount, speedRange);
    ...
    clearPickups();
    clearHyperCountdown();
  }, [initializeAsteroids, generateStarfield, bulletsRef, setBulletCount, shipRef, setUiState, clearPickups, clearHyperCountdown, modeRef, computeWaveSpeedRange]);
      ...
      if (shipRef.current && hyperSpaceJumpEffectRef.current) {
        ...
        hyperSpaceJumpEffectRef.current.trigger(
          ...
          (stageNumber, asteroidCount) => {
            hyperCountdownStageTimeoutRef.current = setTimeout(() => {
              startNewStage(stageNumber, asteroidCount);
              hyperCountdownStageTimeoutRef.current = null;
            }, 100);
          }
        );
